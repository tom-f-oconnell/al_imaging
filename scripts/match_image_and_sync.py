...
def get_exptime(data_dir):
    expxml = join(data_dir, 'Experiment.xml')
...
def get_readable_exptime(data_dir):
    expxml = join(data_dir, 'Experiment.xml')
...
if expdir_envvar in os.environ:
    data_dir = os.environ[expdir_envvar]
    thorsync_dir = data_dir

    # assuming a constant relative position, for now
    print(split(data_dir)[:-1])
    print(join(*split(data_dir)[:-1]))
...
image_dirs =  [d for d in image_dirs \
        if isdir(d) and valid_fly_ids.search(d)]
...
for d, pin2odor in try_decoding.items():
    ...
    try:
        ...
        single_session_acs = read_old_pin2odor(pin2odor)
    ...
    single_session_pl = ta.decode_from_session_directory(d)
    print(' done')
