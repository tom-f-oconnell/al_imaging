...
from al_imaging import util as u
from al_imaging import odors


make_changes = True
check_sorted_thorsync_time = False
# TODO remove this option?
ignore_thorsync = False

# in here should be a directory called 'stimuli'
# which should have pickle files generated by *_odor_randomizer.py

data_dir = u.get_expdir()
if data_dir:
    thorsync_dir = data_dir
    stimuli_base_dir = join(join(*split(data_dir)[:-1]), 'stimuli')

else:
    data_dir = '/media/threeA/Tom/flies'
    stimuli_base_dir = '/media/threeA/Tom/stimuli'
    thorsync_dir = '/media/threeA/Tom/flies'
    '''
    data_dir = 'D:/Hong/Tom/flies'
    stimuli_base_dir = 'D:/Hong/Tom/stimuli'
    thorsync_dir = 'D:/Hong/Tom/flies'
    '''

sync_dirs = [join(thorsync_dir,d) for d in os.listdir(thorsync_dir)]
# TODO redundant with below?
sync_dirs = sorted([d for d in sync_dirs if u.is_thorsync_dir(d)], key=u.get_synctime)

#ordered_sync_dirs = [x for x,y in sorted([(d, get_synctime(d)) \
#        for d in sync_dirs], key=lambda x: x[1])]

image_dirs = [join(data_dir,d) for d in os.listdir(data_dir)]
image_dirs =  [d for d in image_dirs \
        if isdir(d) and u.valid_fly_ids.search(d)]
...
for img_dir in image_dirs:
    print(img_dir)
    fly_id = u.get_fly_id(img_dir)
    
    if u.is_anatomical_stack(img_dir):
        continue
    fly_ids.append(fly_id)

    can_attempt_decoding = False

    if not ignore_thorsync:
        in_img_dir = [join(img_dir,o) for o in os.listdir(img_dir)]
        possible_thorsync_dirs = [d for d in in_img_dir if u.is_thorsync_dir(d)]
        ...
        if len(possible_thorsync_dirs) > 1:
            ...
            # will not be valid after most methods of copy / moving the data
            # to other computers anyway
            if check_sorted_thorsync_time:
                # (in seconds)
                sync_exp_timediff = u.get_synctime(possible_thorsync_dirs[0]) -u.get_exptime(img_dir)
    ...
    # TODO call util funcs
    expxml = join(img_dir, 'Experiment.xml')
    notes = etree.parse(expxml).getroot().find('ExperimentNotes').attrib['text']
    stimfile = u.stimfile_pattern.search(notes)

    # TODO warn if more than one found?
    
    if stimfile is not None:
        stimfile = stimfile.group(0)
        unplaced_stimfile = join(stimuli_base_dir, stimfile)
        
        if not isfile(unplaced_stimfile):
            placed_stimfile = join(img_dir, stimfile)
            if isfile(placed_stimfile):
                stimfile = placed_stimfile

            elif can_attempt_decoding:
                try_decoding[img_dir] = join(join(*split(data_dir)[:-1]), 'pins2odors', stimfile)
                stimfile = None
    ...
    if stimfile is not None:
        ...
        #print('pl_loaded (from pickle)', pl_loaded)
    ...
    pl_match = u.pinlist_re.search(notes)
    pl = None
    ...
    if pl_loaded is not None:
        ...
        if fly_id in fly2pl:
            assert pl_loaded == fly2pl[fly_id], 'apparently conflicting ' + \
                'list of pins used across sessions with one fly_id ' + \
                '(one of sessions information was loaded from pickle): ' + fly_id
        else:
    ...
    all_connections = [u.pin_odor_port_re.findall(n) for n in notes.split('stoppers')[:3]]
...
for fid in fly_ids:
    ...
    fid_indexed_dirs = [(i,d) for i,d in \
        enumerate(sorted(image_dirs, key=u.get_exptime)) \
            if fid in d and not u.is_anatomical_stack(d)]
...
#############################################################################################
# for the session for which we haven't saved a list of the stimuli in advance,
# try to decode the list of pin(s) used on each each trial from the thorsync data
#############################################################################################

print('\ndoing the decoding...')
# TODO check these aren't immediately overwritten in the next step?
# TODO handle better (above, so can check consistency the same way)
for d, pin2odor in try_decoding.items():
    print(d)
    ...
    try:
        ...
        single_session_acs = u.read_old_pin2odor(pin2odor)

    except FileNotFoundError:
        print('did not find pin2odor file ' + pin2odor + ' for ' + d)
        continue

    # TODO check to see if i tend to reference the file in ../../pin2odor
    #print(notes)

    print('decoding pins used from ThorSync data...', end='')
    single_session_pl = u.decode_from_session_directory(d)
    print(' done')
    #print('pl_loaded (from thorsync)', pl_loaded)

    gen_connections = join(d, 'generated_pin2odor.p')
    # TODO make sure format is actually the same here
    if make_changes:
        print('saving valve to odor connections to ' + gen_connections)
        with open(gen_connections, 'wb') as f:
            pickle.dump(single_session_acs[dir2session_idx[d]], f)

    gen_connections = join(d, 'generated_stimorder.p')
    if make_changes:
        print('saving list of pins switched on each trial to ' + gen_connections)
        with open(gen_connections, 'wb') as f:
            pickle.dump(single_session_pl, f)
...
for d in image_dirs:
    ...
    if u.is_anatomical_stack(d):
        continue
    
    fly_id = u.get_fly_id(d)
...
# in data_dir
#
# because the timing information might be on a different computer from the one where you
# currently want to move around your data, it will also print out commands you can run
# on the other computer to do the same move operations
#############################################################################################

if not ignore_thorsync:
    stamped_sync_dirs = [(u.get_synctime(s), s) for s in sync_dirs]
    ...
    for d in sorted(without_thorsync, key=u.get_exptime):
        nearest = min(stamped_sync_dirs, key=lambda x: abs(x[0] - u.get_exptime(d)))
        nearest_thorsync= nearest[1]
            
        if not make_changes:
            print(d)
            print(nearest[0] - u.get_exptime(d))
            print(nearest_thorsync)
            print('')
        
        if abs(nearest[0] - u.get_exptime(d)) < sync_image_mismatch_cutoff:
            to = join(d, split(nearest_thorsync)[-1])
            if make_changes:
...
for d in sync_dirs:
    ...
    h = h5py.File(join(d,'Episode001.h5'), 'r')
    # should be the max?
